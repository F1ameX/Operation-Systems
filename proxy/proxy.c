...
static void handle_client(void *arg) 
{
    ...
    if (n < 0) log_error("client fd=%d: recv failed: %s", cfd, strerror(errno));
    else if (n == 0) log_info("client fd=%d: closed connection immediately", cfd);
    else log_debug("client fd=%d: received %zd bytes (first bytes of request)", cfd, n);
